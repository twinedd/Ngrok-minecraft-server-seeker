...
def create_embed_forfound(list_players, motd, players_online, version, ip):
    ...
    embed = discord.Embed(title="Succesfully founded server",description=f"**Server's description:** {motd}",color=discord.Colour.green(),)
...
async def ping(msg, ip):
    try:
        ...
        await msg.respond('the server is not responding', s)
        pass
